...
const documentsMapping = {
    ...
    'loan offer.pdf': {
        name: 'Loan Offer Document',
        type: 'pdf',
        category: 'loan-offer'
    },
    'loan offer-2.pdf': {
        name: 'Loan Offer - eSign Document',
        type: 'pdf',
        category: 'loan-offer-esign'
    }
};
...
function getCategoryFromFilename(filename) {
    ...
    // Application documents keywords
    if (lower.includes('application form')) {
        return 'application';
    }
...
// Show Loan Offer sections only for CUST0241 after workflow completes
function showLoanOfferSections() {
    const customerId = applicationData?.id;
    if (customerId !== 'CUST0241' || !workflowVisited) return;

    // Show Loan Offer section
    const loanOfferSection = document.getElementById('loanOfferSection');
    if (loanOfferSection) {
        loanOfferSection.style.display = '';
        const grid = document.getElementById('loanOfferGrid');
        if (grid) {
            grid.innerHTML = '';
            const card = createDocumentCard({
                filename: 'loan offer.pdf',
                name: 'Loan Offer',
                officialName: 'Loan Offer Document',
                type: 'pdf',
                url: '/Documents/loan offer.pdf',
                status: 'verified',
                category: 'loan-offer'
            });
    ...
    // Show Loan Offer - eSign section
    const loanOfferEsignSection = document.getElementById('loanOfferEsignSection');
    if (loanOfferEsignSection) {
        loanOfferEsignSection.style.display = '';
        const grid = document.getElementById('loanOfferEsignGrid');
        if (grid) {
            grid.innerHTML = '';
            const card = createDocumentCard({
                filename: 'loan offer-2.pdf',
                name: 'Loan Offer - eSign',
                officialName: 'Loan Offer - eSign Document',
                type: 'pdf',
                url: '/Documents/loan offer-2.pdf',
                status: 'verified',
                category: 'loan-offer-esign'
            });
            grid.appendChild(card);
...
function categorizeAndDisplayDocuments(documents) {
    ...
    documents.forEach(doc => {
        ...
        switch(category) {
            case 'application':
                applicationDocs.push(enrichedDoc);
                break;
            case 'financial':
            ...
            case 'property':
                ...
                break;
            case 'loan-offer':
            case 'loan-offer-esign':
                // These are handled separately in showLoanOfferSections
                break;
            default:
    ...
    renderDocumentCategory('applicationDocumentsGrid', applicationDocs, 'applicationCount');
    ...
    // Show loan offer sections if workflow completed for CUST0241
    showLoanOfferSections();
}
...
function showEmptyDocuments() {
    ['applicationDocumentsGrid', 'kycDocumentsGrid', 'financialDocumentsGrid', 'propertyDocumentsGrid'].forEach(gridId => {
        const grid = document.getElementById(gridId);
        if (grid) {
            grid.innerHTML = `
                <div class="category-empty">
...
function processStep(step) {
    ...
    setTimeout(() => {
        ...
        // Add extra info lines for CUST0241 specific steps
        if (applicationData?.id === 'CUST0241') {
            if (step.id === 'offer-generation') {
                const extraLine = document.createElement('p');
                extraLine.className = 'step-extra-info';
                extraLine.innerHTML = '📄 Offer Generated : <a href="#" onclick="event.preventDefault(); switchTab(\'documents\');" class="step-doc-link">View in Document</a>';
                card.querySelector('.step-content').appendChild(extraLine);
            }
            if (step.id === 'customer-communication') {
                const extraLine = document.createElement('p');
                extraLine.className = 'step-extra-info';
                extraLine.innerHTML = '✅ Sent the loan document and got the eSign from the customer';
                card.querySelector('.step-content').appendChild(extraLine);
            }
        }
